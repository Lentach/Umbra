...
| 40 | 09-26 | (O8) Box media: every device downloads each attachment on arrival and keeps an encrypted local copy (the sender keeps its own), so history outlives the 14-d box TTL; on the PWA the copy is device-local (D9) | OWNER | DONE 09-26 (`BoxMediaStore`/`BoxMediaFetcher`; drive: B prefetched once, IndexedDB held the copy, a reload showed the image with no `/box/media` request) |
| 41 | 09-26 | (O9) Disappearing countdown on box chats, Signal's model: the sender's copies count from the send, each receiving device from the first time it shows the message; an unread message goes 1 d after the send | OWNER | DONE 09-26 (`_startBoxCountdowns`; drive, 30 s timer: A’s row gone by send+53 s, B’s copy kept unopened, counted from the open and gone at open+34 s) |
| 42 | 09-26 | (O10) The chat's timer SETTING stays the server column (`setDisappearingTimer`) until PR4.x; each box message carries its own `ttl` inside E2E and the devices enforce it | OWNER | DONE 09-26 (’setDisappearingTimer` unchanged; each box message carries `ttl`) |
| 43 | 09-26 | (O11) Pings move to the box in item 3 (decision-22 slice) | OWNER | DONE 09-26 (drive: a ping went out as one `/box send`, no ’sendMessage`, B showed the ping effect) |
| 44 | 09-26 | (O12) A box attachment is uploaded ONCE, charged to one peer queue, and every addressed device (peer devices and own siblings) downloads the same id. Accepted residual: the box sees several downloads of one id from different devices and IPs, which links the queues that received it | OWNER | DONE 09-26 (drive: exactly one `POST /box/media`, `box_media` +1, the same id in every frame) |
| 45 | 09-26 | (O13) Pinning a box message is an E2E pin both sides see: pin/unpin travels inside E2E to every peer device and every own sibling, and each device keeps the pin locally. Pinning a box message also clears an old server pin in that chat (one `unpinMessage` emit, which tells the server "the pin changed now" once) | OWNER | ACTIVE (item 4) |
| 46 | 09-26 | (O14) Actions follow the message's path: reactions, pins, edits and deletes of an OLD-path message (a server row) keep today's server events until release N+1 deletes the old tables; box messages use E2E only. Residual: action timing on old messages stays visible to the server | OWNER | ACTIVE (item 4) |
...
| E19a | (Item 4.) Four new envelope types — `t: 'react'`, `'pin'`, `'edit'`, `'del'` — each naming its target by `tg: {s, w}` (the target message's sender account and wire id; unique per sender only). They travel exactly like a box message: one Signal message per live peer device and a SENT COPY (`to`) per live own sibling, all-or-nothing under decision 16. An action that cannot take the box (route gone, box down, refused, silent) FAILS visibly and its optimistic local state reverts; there is no outbox (decisions 19, 25) | One wire shape for all four; the sender-scoped `(s, w)` pair is the only id every device shares for a box message (decision 14) |
| E19b | A receiver resolves `tg` only inside the same chat through the wire-id index. Edit and delete-for-everyone are honoured only when the AUTHENTICATED sender of the action is `tg.s` (today's sender-only rule); a reaction is attributed to the authenticated sender (our own account for a sibling copy); a pin may come from either participant. Order: pin/unpin and edits are last-writer-wins on the action's clamped `ts`; reactions are per `(reactor, emoji)` add/remove, idempotent | The decrypt authenticates the account, so nothing inside the plaintext can claim another sender |
| E19c | Edit: TEXT only, accepted when the edit's clamped `ts` is within 15 min of the target's send time (today's window), content bounded by decision 18; the record is rewritten and `editedAt` set. Delete-for-everyone: destroys the local record and its box media copy, removes the row, clears a pin pointing at it, clears the delivered quote snippet `re.x` of every reply quoting `(s, w)` (the item-3 residual), and keeps a tombstone `(s, w)` for the 30-d box TTL so a message arriving after its delete is dropped. An edit or reaction whose target is not held is dropped | A device may be offline for up to the TTL; the tombstone is the only way a late copy cannot resurrect a deleted message |
| E19d | Reactions on a box message are plain emoji inside E2E, kept in the target's plaintext record so they survive a restart; the server's blinded reaction tokens (`K_react`) stay for old-path rows only (decision 46) | The box never sees the envelope, so blinding buys nothing there |
