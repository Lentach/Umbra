...
**Date:** 2026-09-25 · **Branch:** `feat/metadata-privacy` at `a35930e4` · **Decision log:** `metadata-privacy-decisions.md` (S8: owner answers only the OWNER questions below; the ENGINEERING calls are the agent's, with reasons, overruled at the gate if wrong).

## What is left

| # | Work | Built on | Notes |
|---|---|---|---|
| 1 | **Sibling queues part B**: sent copies to own devices over the box, rotation on revoke, pruning, no-session policy | part A (`a35930e4`) | unblocks multi-device senders on the box |
| 2 | **Box push registration** | 1 | a box-only message wakes no closed app until this lands (23) |
| 3 | **Slice (e)**: `device_added` / `device_removed` / `list_update` over the box | (b), (c) | replaces the server's `staleLists` bounce |
| 4 | **Slice (d)**: migration handoff over the old path | (c) | moves existing friendships onto the box |
| 5 | **Slice (f)**: first contact over request queues | (a), part A frames | friend requests over the box |
| 6 | **Decision-22 batch**: replies, media, disappearing timers; reactions, pin, edit, delete-for-everyone over E2E | 1–5 | the reaction bar on a box message fails today |
| 7 | **Slice (g)**: receipts and typing (D5) | 6 | |
| 8 | **Prod prerequisites** for `BOX_ENABLED`: global ceiling, per-socket rid cap, media refusals counted | — | backend only, runs in parallel with 3–7 |
| 9 | **G5**: gate review, migration rehearsal on a device (master APK, then the branch APK over it), then release N | 1–8 | owner gate |

Order follows decisions 17, 22 and 23. Frontend slices share `messaging_provider.box.dart`, so they go one at a time. Item 8 is backend-only and runs alongside them.
...
- **E6. Rotation.** When the own verified list stops naming a device (identity `deviceListChanged`, then re-verify), each surviving device:
  ...
  3. deletes the old queue once the handoffs are acked, or at the next connect.

  Idempotent per connect. *Reason:* decision 27. Only the survivors can act, since the revoked device learns nothing.
- **E7.** On every own-list change, `boxsib_v1` entries for devices that are no longer live are dropped. *Reason:* the verified list is the authority (E2).
- **E8.** A sibling no-session follows the peer failure policy. It is held only on the self-queue and bounded by the journal's existing 30-day prune. *Reason:* one policy, and the request-queue half is already fixed (E1–E3).
