...
- **Contact backup, what it does and does not leak (PR2.4):** the row's `blob` is AES-256-GCM under a content key the server never receives; the content key is itself wrapped under PBKDF2-HMAC-SHA256(600 000) of the account password and, when the user has enrolled one, of the recovery phrase. The server therefore learns three things and no more: that an account has a backup, roughly how large it is, and when it last CHANGED. `updatedAt` deliberately does NOT move for a re-upload of identical bytes — the same rule that stopped `key_bundles.updatedAt` being a per-device presence clock — so an idle reconnect leaves no trace. The blob deliberately excludes this device's queue private keys and the server-side conversation/request ids.
