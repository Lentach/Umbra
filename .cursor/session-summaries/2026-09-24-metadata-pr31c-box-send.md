...
- Box tempIds are pinned once a frame went out (`_boxTempIds`: no old-path retry, token kept across a same-user reconnect); in-flight box sends (`_boxInFlight`) are skipped by `markSendingMessagesFailed`; the peer list is re-verified once per connect then ≤ every 10 min.
...
## Verification
- TDD: 6/13 send tests RED on behaviour before the route existed; frame-fit test RED at 46 214 Signal B under the old 45 000 limit, GREEN at 14 000 (real libsignal PreKey = 15 220 B ≤ 16 315; plaintext 15 072, overhead 148).
- Mutants: 35 across four runs, 32 killed; 2 survivors were dead conditions and were deleted (`mediaUrl == null` under the TEXT check; the store-open check before `byUserId`); 1 timing-only survivor accepted (an extra turn before `_boxRoute` for a box-capable peer). Files hash-identical after every run.
- Review `Pr31cReview` REQUEST_CHANGES (P1 stale cached peer list, P2 in-flight retry duplicate, P2 unbounded link preview, P3 pin before any frame) + advisor blocker (a READY-gated `addressesFor` leaked covered peers to the old path while the box was down): all fixed test-first, each with a killed mutant.
...
- Owed: a box-pinned retry whose route is gone stays failed for good; a crash between the box accepting and the sent record's write loses the sender's copy; linked/revoked peer devices are seen only at the next re-verify (≤ 10 min) until slice (e).
- Traps (also in `docs/agents/traps.md`): an extra `await` before the old-path emit breaks `chat_input_bar_attachment_test`; a box route must never depend on the box being connected; a same-user reconnect clears `_sendTokenByTempId`.
