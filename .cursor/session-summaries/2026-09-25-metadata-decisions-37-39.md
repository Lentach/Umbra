...
## Verification
- Decision 39: a throwaway `vm` harness ran the REAL SW source with a fake `self` over 8 cases. With a focused chat: Apple posts silent then closes; FCM posts nothing; a failed `getSubscription` posts then closes. With an unfocused chat: FCM and Apple both post a loud card that stays. `notifier_challenge`: nothing on a visible page on FCM; a flash on Apple, or when hidden. Red run (2026-09-26) on the pre-change file (`78a4f36c~1`): Apple+focused and failed-subscription+focused posted NOTHING; the challenge on Apple already flashed (control).
