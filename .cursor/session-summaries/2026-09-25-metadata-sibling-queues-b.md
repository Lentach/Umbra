...
- Handoff skills (after a blind quiz found fresh agents missing ~40% of this session): `umbra-session-start` §3 always reads the dated summary whole, traps by `##` group, CI on the baton's sha; §2 staleness by summary name and a no-baton step 3; `umbra-session-end` template adds a CI line, the next action, out-of-repo changes, survivor fates, recipes and a ~700-char line cap; the summary owns the next action and a post-push CI result is committed before the baton; AGENTS.md #3 and CLAUDE.md session-start line aligned.
...
## Verification
- CI: NOT RUN on the code commit `59e141de` (0 check-runs) nor on the handoff commits (`21cf61bf`: only CodeQL). Draft PR #185 is CONFLICTING with master: the only conflict is `.cursor/session-summaries/LATEST.md` vs master's docs commit `2acd49b1` (`git merge-tree`), so the rebase is small.
...
- Handoff quiz: 14 questions, blind agents with file:line citations. Before: strict start ~8.5/14, no-baton ~10/14 with the WRONG next action. After the skill + summary fixes: 14/14 on both, every answer from a tracked file.
