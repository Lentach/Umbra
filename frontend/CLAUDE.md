...
- Web Push and silence: on an Apple endpoint (`web.push.apple.com`) EVERY push must post a notification inside `waitUntil` — Safari REVOKES the subscription after 3 silent ones; elsewhere a push may stay silent only while a visible page of ours is present. The box `notifier_challenge` (E9) follows this. The old `new_message` path does NOT: `shouldSuppressForFocusedConversation` skips the banner on every endpoint, Apple included (open, 2026-09-25). Never add a data-only branch (`docs/contracts/wire.md`, box bullet "Client push registration (E9)").
