...
- Tests: `cd backend && npm test` (1159 unit tests, 64 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (2192 Flutter tests, 14 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs`). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — 46 passed / 14 skipped, MEASURED locally 2026-09-17 (the 14 = the 9 pre-existing opt-ins + the 5 of the gated encryption proof). `encryption_proof_test.dart` is NOT in this run: its 5 tests are gated on `--dart-define=ENCRYPTION_PROOF=true` and run on the `e2e-isolated-probes` stack, because its 2 registrations tipped the shared `/auth/register` bucket into a 429 on `f86a43de` (E2E_DB_CONTAINER defaults to this checkout's fireplace-db-1; CI resolves it dynamically); see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job below. Origin-wide Web Lock probe: `node scripts/verify-session-lock-probe.mjs` (also a CI job). **The §6.2 reset teardown + falsification 12 probe is OPT-IN** — `flutter test test_e2e/identity_reset_teardown_test.dart --dart-define=RESET_PROBE=true` — because it spends the shared `/auth/register` bucket; it runs in CI as its own `e2e-isolated-probes` job.
